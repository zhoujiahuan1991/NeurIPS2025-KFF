...
def load_imagenetc(
    ...
    assert offset + n_examples <= 5000, f"offset + n_examples should be less than 5000, offset: {offset}, n_examples: {n_examples}"
    ...
    test_loader = data.DataLoader(imagenet, batch_size=n_examples+offset,
                                  shuffle=shuffle, num_workers=2)

    x_test, y_test, paths = next(iter(test_loader))
    x_test = x_test[offset:]
    y_test = y_test[offset:]
    # print(f'first path: {paths[offset]}')
